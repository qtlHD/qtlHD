...
unittest {
  ...
  foreach(i; 0..genoprobs.length)
    foreach(j; 0..1)
      assert(abs(genoprobs[i][0][j] - genoprobs_from_rqtl[i][j]) < 1e-6);
  ...
  rec_frac_from_rqtl = [0.0000000000010000000000000001818,
  ...
  auto rec_frac_rev = estmap_BC(genotype_matrix, chr15_map, rec_frac, 0.001, 100, 1e-6, true);
