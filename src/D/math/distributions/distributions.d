...
unittest {
  double[] ppois01 = [0.90483741803595951758, 0.99532115983955560257, 0.99984534692973536796, 0.99999615316607459725, 0.99999992332198317513,
                      0.99999999872510125787, 0.99999999998181987593, 0.99999999999977307041, 0.99999999999999755751, 1.00000000000000000000,
                      1.00000000000000000000];
  double[] ppois1 = [0.36787944117144233402, 0.73575888234288466805, 0.91969860292860583506, 0.98101184312384615005, 0.99634015317265633982,
                     0.99940581518241833336, 0.99991675885071196195, 0.99998975080332530574, 0.99999887479740201535, 0.99999988857452171143,
                     0.99999998995223360332];
  double[] ppois5 = [0.0067379469990854670008, 0.0404276819945128054745, 0.1246520194830811611997, 0.2650259152973618026472,
                     0.4404932850652122922064, 0.6159606548330631703436, 0.7621834629729387078356, 0.8666283259299926156061,
                     0.9319063652781515161294, 0.9681719426937951400447, 0.9863047314016171185358];

  foreach(i; 0..10) {
    assert(abs(ppois(i, 0.1) - ppois01[i]) < 1e-12);
    assert(abs(ppois(i, 1.0) - ppois1[i]) < 1e-12);
    assert(abs(ppois(i, 5.0) - ppois5[i]) < 1e-12);

    assert(abs(ppois(i, 0.1, false) - (1.0-ppois01[i])) < 1e-12);
    assert(abs(ppois(i, 1.0, false) - (1.0-ppois1[i])) < 1e-12);
    assert(abs(ppois(i, 5.0, false) - (1.0-ppois5[i])) < 1e-12);
  }
